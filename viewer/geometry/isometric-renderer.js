...
export class IsometricRenderer {
  constructor(canvasContainer) {
    ...
    on('viewer3d-settings-changed', (s) => this._applySettings(s));
  }

  _applySettings(s) {
      if ((s.projection === 'orthographic') !== this._isOrtho) {
          this.toggleProjection();
      }
      this._perspCamera.fov = s.fov;
      this._perspCamera.near = Math.max(1e-4, s.nearPlane);
      this._perspCamera.far = s.farPlane;
      this._perspCamera.updateProjectionMatrix();

      this._controls.enableDamping = s.dampingFactor > 0;
      this._controls.dampingFactor = s.dampingFactor;
      this._controls.rotateSpeed = s.rotateSpeed;
      this._controls.panSpeed = s.panSpeed;
  ...
  _applyAxisConvention(convention) {
      if (convention === 'Z-up') {
          // Z-up: Rotate world so Z becomes up. Camera up is (0,0,1).
          this._sceneRoot.rotation.x = -Math.PI / 2;
          this._camera.up.set(0, 0, 1);
      } else {
          // Y-up: Native Three.js.
  ...
  _init() {
    ...
    // Apply default fov, near, far from settings
    const s = state.sticky.viewer3d;
    this._perspCamera = new THREE.PerspectiveCamera(s.fov, aspect, Math.max(1e-4, s.nearPlane), s.farPlane);

    this._isOrtho = (s.projection === 'orthographic');
    ...
    this._applyAxisConvention(state.sticky.viewer3d.axisConvention);
  ...
  _snapCamera([cx, cy, cz], [ux, uy, uz]) {
    ...
    if (state.sticky.viewer3d.axisConvention === 'Z-up') {
        const qRoot = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1,0,0), -Math.PI/2);
        offset.applyQuaternion(qRoot);
        up.applyQuaternion(qRoot);
    }

    this._camera.position.copy(centre).add(offset);
  ...
  _syncViewCube() {
    if (!this._viewCubeEl || !this._camera) return;
    if (!state.sticky.viewer3d.showViewCube) {
        this._viewCubeEl.style.display = 'none';
        return;
    }
    this._viewCubeEl.style.display = '';

    if (!this._viewCubeInner) return;

    // ViewCube needs to rotate relative to the scene root
    const qCam = this._camera.quaternion.clone();

    if (state.sticky.viewer3d.axisConvention === 'Z-up') {
        const qRoot = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1,0,0), -Math.PI/2);
        qCam.multiply(qRoot.invert());
    }

    this._viewCubeInner.style.transform =
      `matrix3d(${new THREE.Matrix4().makeRotationFromQuaternion(qCam.invert()).elements.join(',')})`;
  }
  ...
  _syncAxisGizmo() {
    ...
    if (!state.sticky.viewer3d.showAxisGizmo) {
    ...
    let AXES;
    if (isZUp) {
         // Scene is rotated -90 around X.
         // Real World Z is now pointing along ThreeJS -Y
         AXES = [
            { dir: new THREE.Vector3(1, 0, 0),  color: '#33cc33', neg: '#226622', label: 'Y' }, // North
            { dir: new THREE.Vector3(0, 1, 0),  color: '#3388ff', neg: '#224488', label: 'Z' }, // Up (In Three.js Space after rotation this points "Up")
            { dir: new THREE.Vector3(0, 0, 1),  color: '#ff3333', neg: '#882222', label: 'X' }, // East
        ];
    } else {
        AXES = [
            { dir: new THREE.Vector3(1, 0, 0),  color: '#ff3333', neg: '#882222', label: 'X' },
            { dir: new THREE.Vector3(0, 1, 0),  color: '#33cc33', neg: '#226622', label: 'Y' },
            { dir: new THREE.Vector3(0, 0, 1),  color: '#3388ff', neg: '#224488', label: 'Z' },
        ];
    }


    // Project all axes and sort back-to-front (draw behind first)
    // Important: we need to apply the scene root's rotation to the axis direction
    // before applying the camera's quaternion.
    const projected = AXES.map(({ dir, color, neg, label }) => {
      // The gizmo is in camera space. The camera looks at the rotated scene.
      // So we must rotate the standard basis vectors by the same rotation we apply to the scene.
      const worldDir = dir.clone();
      if (isZUp) {
          worldDir.applyAxisAngle(new THREE.Vector3(1,0,0), -Math.PI / 2);
      }

      const proj = worldDir.applyQuaternion(this._camera.quaternion.clone().invert());
      // Invert Y projection because canvas Y is down, ThreeJS Y is up
      return { proj: new THREE.Vector3(proj.x, -proj.y, proj.z), color, neg, label, depth: proj.z };
    });
    ...
    for (const { proj, color, neg, label } of projected) {
      ...
      const ey = cy + proj.y * len; // proj.y is already inverted
  ...
  toggleProjection() {
      ...
      if (this._isOrtho) {
          ...
          // Match ortho frustum height to perspective visible height at target dist
          const fovRad = THREE.MathUtils.degToRad(this._perspCamera.fov);
          const orthoHalfHeight = Math.tan(fovRad / 2) * dist;
  ...
  _fitToScene() {
    ...
    // Auto-fit near/far if enabled
    if (state.sticky.viewer3d.autoNearFar) {
       this._autoFitNearFar(center, maxDim);
    }
    ...
    if (this._isOrtho) {
        ...
        // Compute distance needed to fit bounding sphere into FOV
        const radius = maxDim / 2;
        const fov = this._perspCamera.fov * (Math.PI / 180);
        let dist = radius / Math.sin(fov / 2);

        // Adjust for aspect ratio if height is the constraint
        if (aspect < 1) {
             dist = dist / aspect;
        }

        this._camera.position.copy(center).add(new THREE.Vector3(1, 1, 1).normalize().multiplyScalar(dist));
    }

    this._controls.update();
  }

  _autoFitNearFar(targetCenter, maxDim) {
      const dist = this._camera.position.distanceTo(targetCenter);
      const radius = maxDim / 2;

      this._camera.near = Math.max(1e-4, (dist - radius) * 0.1);
      this._camera.far  = (dist + radius) * 10;
      this._camera.updateProjectionMatrix();
  }
